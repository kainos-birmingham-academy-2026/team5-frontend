...
test.describe("Job role detail", () => {
	...
	test("opens the detail page for the first advertised role", async ({
		...
		test.skip(
			(await jobRoleListPage.jobCount()) === 0,
			"No job roles available in this environment.",
		);
	...
	test("matches the role returned by the API", async ({
		...
		test.skip(role === null, "API did not return any job roles.");
	...
	test("navigates back to the list", async ({ jobRoleDetailPage, page }) => {
