...
export class JobRoleController {
	...
	async getJobRoleById(req: Request, res: Response): Promise<void> {
		...
		try {
			...
			if (!jobRole) {
				res.status(404).render("job-role-detail.html", { jobRole: null });
				return;
			}

			res.render("job-role-detail.html", { jobRole });
		} catch (error) {
