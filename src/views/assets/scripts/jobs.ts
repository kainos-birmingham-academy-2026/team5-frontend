...
if (siteHeader && navToggle) {
  (navToggle as HTMLElement).addEventListener("click", () => {
    const isOpen = (navToggle as HTMLElement).getAttribute("aria-expanded") === "true";
...
if (form) {
  const keywordInput = (form as HTMLFormElement).querySelector('[data-filter="keyword"]') as HTMLInputElement | null;
  const locationInput = (form as HTMLFormElement).querySelector('[data-filter="location"]') as HTMLInputElement | null;
  const cards: HTMLElement[] = [...(document.querySelectorAll(".job-card") as NodeListOf<HTMLElement>)];
